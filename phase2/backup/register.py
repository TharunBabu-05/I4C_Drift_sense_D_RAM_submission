...
====================================================================
Usage:
    python register.py --input <input_csv_path> --output <output_csv_path>

Inputs:
    pairs.csv containing columns: pair_id, reference_path, search_path

Outputs:
    predictions.csv containing columns: pair_id, x, y, theta, scale, found, score

Rules Compliance:
    - 100% CPU Execution
    - Zero external network dependencies
    - Evolves Round 1 Hybrid NCC + Custom 4-Layer ResNet Siamese Network
"""
...
# Enforce single-threaded execution on Windows CPU to prevent paging file size limits
...
# Ensure phase2 directory is in Python path
...
def main():
    ...
    # Initialize Pyramidal Multi-Scale & Multi-Rotation Siamese Engine
    checkpoint_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "best_model_level1.pth")
    if not os.path.exists(checkpoint_path):
        print(f"Error: Pre-trained weight file '{checkpoint_path}' not found.")
        sys.exit(1)
    ...
    with open(input_csv, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            try:
                ...
                # Fallback on unexpected error: output rejected safely
                predictions.append({
                    "pair_id": pair_id,
                    "x": "0.0000",
                    "y": "0.0000",
    ...
    # Output predictions in exact required schema
