#!/usr/bin/env python3
"""
EXP-20 — Evaluation of Model Checkpoint: checkpoints_phase2_v2_best/best_model_phase2.pth
========================================================================================

Evaluates checkpoints_phase2_v2_best/best_model_phase2.pth on the official 200-pair Phase-2 benchmark:
- File Size: 1,532,893 bytes
- Training: 16k dataset with joint scale/rot heads (lambda_scale=0.3, lambda_rot=0.1)
- Validation Accuracy: 99.71%
- Production Engine: phase2/phase2_inference.py (EXP-13 Periodicity Penalization)

Compare score against verified production baseline (72.80 / 100.0).
"""
...
def load_checkpoint_flexibly(engine, ckpt_path):
    state_dict = torch.load(ckpt_path, map_location=engine.device)
    # Check if state_dict has matching keys
    ...
    for k, v in state_dict.items():
        if k in model_state and model_state[k].shape == v.shape:
            ...
            filtered_state[k] = v
    engine.model.load_state_dict(filtered_state, strict=False)
    print(f"[OK] Flexible checkpoint loaded: {len(filtered_state)} / {len(model_state)} layers matched.")

def main():
    print("=" * 70)
    print("EVALUATING MODEL: checkpoints_phase2_v2_best/best_model_phase2.pth")
    print("=" * 70)

    ckpt_path = "checkpoints_phase2_v2_best/best_model_phase2.pth"
    assert os.path.exists(ckpt_path), f"Checkpoint not found: {ckpt_path}"
    ckpt_size = os.path.getsize(ckpt_path)

    with open(ckpt_path, "rb") as f:
        sha = hashlib.sha256(f.read()).hexdigest()
    print(f"[OK] Checkpoint File Size: {ckpt_size} bytes")
    print(f"[OK] Checkpoint SHA-256:  {sha}")

    engine = Phase2InferenceEngine(checkpoint_path=None, device="cpu")
    load_checkpoint_flexibly(engine, ckpt_path)
    ...
    print(f"\n=======================================================")
    print("BEST CHECKPOINT (checkpoints_phase2_v2_best) 100-POINT SCORE")
    print("=======================================================")
    ...
    print(f"-------------------------------------------------------")
    print(f"TOTAL SCORE:  {metrics['total_100_score']:.2f} / 100.0 (Baseline was 72.80)")
