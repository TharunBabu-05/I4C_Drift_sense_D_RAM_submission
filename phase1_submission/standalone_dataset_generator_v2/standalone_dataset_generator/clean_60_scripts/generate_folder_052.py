#!/usr/bin/env python3
"""
Generate Pair 2 for Hybrid Single-Zigzag Line + Box Capacitor DRAM (Top GT 500,310)
==================================================================================
Synthesizes 1000x1000 Grayscale SEM image matching Hybrid DRAM Architecture:
  ...
  - Stored in Script_and_their_images/052/
"""
...
def render_hybridtwist_dram_scene(h=1000, w=1000, gt_x=500, gt_y=310, seed=21002):
...
# [STRIPPED] def add_unique_sem_noise(image_uint8, seed=21002, is_target=False):
...
# [STRIPPED] 
# [STRIPPED] 
def generate_hybridtwist_pair2(output_dir="generated_hybridtwist/pair2", gt_x=500, gt_y=310, seed=21002):
    h, w = 1000, 1000
    ...
    gt_info = {
        "center_x": gt_x,
        "center_y": gt_y,
        "target_name": "Grayscale Hybrid Single-Zigzag & Box-Capacitor DRAM - Top GT 500,310 (Pair 2)",
        "pair_id": "pair2",
        "scale_factor": 10.0
    }
    ...
    print(f"[generate_hybridtwist_pair2] Successfully generated Hybrid DRAM pair 2 {output_dir} | GT: ({gt_x}, {gt_y})")
    return os.path.join(output_dir, "search.png"), os.path.join(output_dir, "target.png"), gt_info


if __name__ == "__main__":
    generate_hybridtwist_pair2()
