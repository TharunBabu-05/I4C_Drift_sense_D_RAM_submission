#!/usr/bin/env python3
"""
Generate Pair 1 for generated_new (Grayscale Dense Hexagonal Cell DRAM - Center GT 500,500)
============================================================================================
...
def render_hexagonal_dram_scene(h=1000, w=1000, gt_x=500, gt_y=500, seed=4001):
    np.random.seed(seed)
    img = np.full((h, w), 28, dtype=np.float32)

    # --- Dense layout parameters ---
    col_pitch = 16   # tighter horizontal spacing (was 24)
    row_pitch = 12   # tighter vertical spacing  (was 16)
    hex_r     = 5    # hex outer radius
    via_r     = 2    # inner via ring radius

    for cx in range(8, w - 8, col_pitch):
        col_type = (cx // col_pitch) % 3

        if col_type == 0:
            # Hex Cell Array column
...
# [STRIPPED] def add_heavy_sem_noise(image_uint8, seed=4001, is_target=False):
# [STRIPPED]     np.random.seed(seed)
# [STRIPPED]     std = 20 if not is_target else 8      # much heavier noise
...
# [STRIPPED] 
# [STRIPPED] 
def generate_new_pair1(output_dir="generated_new/pair1", gt_x=500, gt_y=500, seed=4001):
    h, w = 1000, 1000
    os.makedirs(output_dir, exist_ok=True)

    search_clean = render_hexagonal_dram_scene(h=h, w=w, gt_x=gt_x, gt_y=gt_y, seed=seed)

    # Anchor crop to exact trace midpoint (between the two snapped via centres)
    col_pitch, row_pitch = 16, 12
    col_idx = round((gt_x - 8) / col_pitch)
    if col_idx % 3 != 0:
        col_idx = (col_idx // 3) * 3
    row_idx = round((gt_y - 8) / row_pitch)
    anchor_x = 8 + col_idx * col_pitch
    anchor_y = 8 + row_idx * row_pitch + row_pitch // 2   # midpoint between the two vias
    ...
    gt_info = {
        ...
        "target_name": "Grayscale Dense Hexagonal Cell DRAM (Embedded Inter-Cell Trace) - Center GT 500,500 (Pair 1)",
        "pair_id": "pair1", "scale_factor": 10.0
    }
    ...
    for sub in ["generated_wafer/pair1"]:
    ...
    print(f"[pair1] Done | GT: ({gt_x}, {gt_y})")
