#!/usr/bin/env python3
"""
Generate Pair 2 for generated_new (High-Density SEM Transistor Array - Ultra-Heavy Multi-Noise + Raster Banding)
==============================================================================================================
...
Design Features:
  ...
  - UNIQUE TARGET LANDMARK: At Ground Truth (420, 580), ONLY THE BOTTOM TWO PARALLEL CAPACITOR LINES
...
def render_dense_sem_memory_array(h=1000, w=1000, gt_x=420, gt_y=580, seed=1102):
    """
    ...
    # UNIQUE TARGET LANDMARK: Solid CIRCLE via pad with accent ring at (gt_x, gt_y)
    rad = 12
    cv2.circle(img, (gt_x, gt_y), rad, 255, -1)
    cv2.circle(img, (gt_x, gt_y), 4, 38, -1)
    cv2.circle(img, (gt_x, gt_y), 16, 230, 2)
...
# [STRIPPED] def add_heavy_multi_sem_noise(image_uint8, seed=1102, is_target=False):
# [STRIPPED]     """
...
# [STRIPPED] 
# [STRIPPED] 
def generate_new_pair2(output_dir="generated_new/pair2", gt_x=420, gt_y=580, seed=1102):
    """Generate dense SEM pair2 with ultra-heavy independent noise applied to BOTH search and target images."""
    h, w = 1000, 1000
    ...
    # Extract 100x100 crop around GT center (420, 580) and scale 10x to 1000x1000 px for target.png
    crop_size = 100
    ...
    gt_info = {
        "center_x": gt_x,
        "center_y": gt_y,
        "target_name": "Dense SEM Memory Array with Ultra-Heavy Multi-Noise & Banding (Target & Search) 2",
        "pair_id": "pair2",
        "scale_factor": 10.0
    }
    ...
    print(f"[generate_new_pair2] Successfully generated ultra-heavy noisy target & search pair {output_dir} | GT: ({gt_x}, {gt_y})")
