#!/usr/bin/env python3
"""
Generate Pair 1 for generated_fbc (Grayscale Floating-Body Cell DRAM - Center GT 500,500)
========================================================================================
Synthesizes 1000x1000 Grayscale SEM image matching FBC (Floating-Body Cell) DRAM structure:
  ...
  - Seamless Ground Truth Target Landmark at Center (500, 500): Cluster of adjacent FBC cells
...
def render_fbc_dram_scene(h=1000, w=1000, gt_x=500, gt_y=500, seed=11001):
        ...
        for dr in range(4):
            ...
            if (cur_x - fin_w // 2 >= 0 and cur_x + fin_w // 2 < w and
                ...
                if dr < 3:
                    ...
                    cv2.line(img_gray, (cur_x, y2), (cur_x, next_y - gate_h // 2), 245.0, 2)
...
# [STRIPPED] def add_sem_noise_grayscale(image_uint8, seed=11001, is_target=False):
...
# [STRIPPED] 
# [STRIPPED] 
def generate_fbc_pair1(output_dir="generated_fbc/pair1", gt_x=500, gt_y=500, seed=11001):
    h, w = 1000, 1000
    ...
    gt_info = {
        "center_x": gt_x,
        "center_y": gt_y,
        "target_name": "Grayscale Floating-Body Cell (FBC) DRAM - Center GT 500,500 (Pair 1)",
        "pair_id": "pair1",
        "scale_factor": 10.0
    }
    ...
    print(f"[generate_fbc_pair1] Successfully generated FBC DRAM pair 1 {output_dir} | GT: ({gt_x}, {gt_y})")
