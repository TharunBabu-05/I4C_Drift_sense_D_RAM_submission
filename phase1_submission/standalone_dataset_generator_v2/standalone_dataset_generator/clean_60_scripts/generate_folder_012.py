#!/usr/bin/env python3
"""
Generate Pair 2 for generated_new (Silicon Wafer Multi-Chip Array - Target Chip at Top-Center 500,335 - ANOTHER CHIP!)
======================================================================================================================
...
Design Features:
  ...
  - Target Chip at Top-Center of Wafer (500, 335) -- ANOTHER CHIP LOCATION THAN PAIR 1!
  ...
  - UNIQUE TARGET LANDMARK (Pair 2 - Top-Center Chip 500,335): Boxes remain 100% separate individual squares,
    joined by THIN METALLIC INTERCONNECT TRACE LINES running through the 4 rows of the box matrix.
  - Independent SEM noise applied to search.png and target.png (Seed 2002).
  - Razor-sharp edges with nearest-neighbor scaling for target.png.

Files created in generated_new/pair2/ (and copied to generated_wafer/pair2/):
  - groundtruth.json
  - search.png    (1000x1000 px grayscale wafer image with sharp thin trace joined boxes & noise)
  - target.png    (1000x1000 px grayscale 100x high-mag target image centered at 500, 335)
...
def render_semiconductor_wafer_scene(h=1000, w=1000, gt_x=500, gt_y=335, seed=2002):
    """
    Render 1000x1000 uint8 grayscale Semiconductor Wafer multi-chip layout for Pair 2:
    ...
    - Target chip at ANOTHER CHIP LOCATION (gt_x, gt_y) = (500, 335) [Top-Center Chip]: Boxes remain separate individual squares, joined by THIN METALLIC TRACE LINES
    """
    np.random.seed(seed)
    ...
    for cy in rows:
        for cx in cols:
            ...
            if np.sqrt((cx - center_wafer[0])**2 + (cy - center_wafer[1])**2) < radius_wafer - 55:
                ...
                # PAIR 2 UNIQUE TARGET LANDMARK AT ANOTHER CHIP (500, 335):
                # Thin metallic trace lines joining adjacent box rows (boxes remain 100% separate individual squares!)
                is_gt_chip = (cx == gt_x and cy == gt_y)
                if is_gt_chip:
                    b0_x = gt_x - 52
                    b0_y = gt_y - 52
                    b3_x = b0_x + 3 * box_pitch

                    # Thin metallic horizontal trace lines across all 4 rows
                    for r in range(4):
                        cv2.line(img, (b0_x, b0_y + r * box_pitch), (b3_x, b0_y + r * box_pitch), 255, 3)
...
# [STRIPPED] def add_heavy_multi_sem_noise(image_uint8, seed=2002, is_target=False):
# [STRIPPED]     """
...
# [STRIPPED] 
# [STRIPPED] 
def generate_new_pair2(output_dir="generated_new/pair2", gt_x=500, gt_y=335, seed=2002):
    """Generate pair2 with Target Chip at Top-Center (500, 335) - ANOTHER CHIP!"""
    h, w = 1000, 1000
    ...
    # Extract 100x100 crop around GT center (500, 335) and scale 10x to 1000x1000 px for target.png
    crop_size = 100
    ...
    gt_info = {
        "center_x": gt_x,
        "center_y": gt_y,
        "target_name": "Silicon Wafer Multi-Chip Array (Thin Line Trace Joined Boxes - Top-Center Chip 500,335) 2",
        "pair_id": "pair2",
        "scale_factor": 10.0
    }
    ...
    # Also sync to generated_wafer/pair2 for seamless compatibility
    ...
    print(f"[generate_new_pair2] Successfully generated pair 2 {output_dir} | GT: ({gt_x}, {gt_y})")
