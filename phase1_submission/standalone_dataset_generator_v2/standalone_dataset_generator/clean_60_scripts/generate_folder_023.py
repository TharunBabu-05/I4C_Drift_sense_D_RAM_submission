#!/usr/bin/env python3
"""
Generate Pair 1 for generated_7fsquare (Grayscale 7F² DRAM Cell Layout - Center GT 500,500)
============================================================================================
Synthesizes 1000x1000 Grayscale normal image matching SEM micro-photo of 7F² DRAM IC structure:
  ...
  - Seamless Ground Truth Target Landmark at Center (500, 500): Bright cluster of 7F² cells
...
def render_7fsquare_dram_scene(h=1000, w=1000, gt_x=500, gt_y=500, seed=7001):
    ...
    # Two wordlines per cell-row: storage WL and passing WL
    wl_pitch = cell_h  # 7 px per wordline pair row
    for wy in range(0, h, wl_pitch):
        # Storage wordline (brighter, connects gate to capacitor)
        storage_wl_y = wy + 2
        if 0 <= storage_wl_y < h:
            cv2.line(img_gray, (0, storage_wl_y), (w, storage_wl_y), 105.0, 1)
        # Passing wordline (dimmer, passes over without connecting)
        passing_wl_y = wy + 5
        if 0 <= passing_wl_y < h:
...
def apply_vignette(img_f32, strength=0.35):
    ...
    # Cosine-4th-power vignette model (optical physics)
...
def apply_barrel_distortion(img_u8, k1=0.12, k2=0.04):
    ...
    # Compute undistort maps and remap (applies barrel distortion)
...
def apply_scan_line_noise(img_f32, intensity=3.0, freq=0.02):
    ...
    # Sinusoidal ripple + random per-line offset
...
# [STRIPPED] def add_heavy_sem_noise_grayscale(image_uint8, seed=7001, is_target=False):
...
# [STRIPPED]         # Search image: heavier noise (lower magnification, more degradation)
...
# [STRIPPED]     else:
# [STRIPPED]         # Target image: lighter noise (higher magnification, cleaner)
...
# [STRIPPED] 
# [STRIPPED] 
def generate_7fsquare_pair1(output_dir="generated_7fsquare/pair1", gt_x=500, gt_y=500, seed=7001):
    h, w = 1000, 1000
    ...
    gt_info = {
        "center_x": gt_x,
        "center_y": gt_y,
        "target_name": "Grayscale 7F² DRAM Cell Layout - Center GT 500,500 (Pair 1)",
        "pair_id": "pair1",
        "scale_factor": 10.0
    }
    ...
    print(f"[generate_7fsquare_pair1] Successfully generated 7F² DRAM pair 1 {output_dir} | GT: ({gt_x}, {gt_y})")
    return os.path.join(output_dir, "search.png"), os.path.join(output_dir, "target.png"), gt_info


if __name__ == "__main__":
    generate_7fsquare_pair1()
