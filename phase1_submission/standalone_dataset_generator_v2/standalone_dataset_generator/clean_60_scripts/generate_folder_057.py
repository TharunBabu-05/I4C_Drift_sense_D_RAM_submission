#!/usr/bin/env python3
"""
Generate Pair 1 for Grayscale Asymmetric Dual-Block DRAM (Center GT 500,500)
============================================================================
Synthesizes 1000x1000 Grayscale SEM image matching Asymmetric Dual-Block DRAM:
  ...
  - Stored in Script_and_their_images/057/
"""
...
def render_asymblock_dram_scene(h=1000, w=1000, gt_x=500, gt_y=500, seed=24001):
    ...
    for cx in range(col_pitch_a // 2, 480, col_pitch_a):
        ...
        for wy in range(row_pitch // 2, h - 40, row_pitch):
            ...
            cs_y = wy + row_pitch // 2
            if cs_y + 8 < h - 40:
                # Left-aligned Storage Capacitor (Block A style)
    ...
    for col_idx, cx in enumerate(range(520 + col_pitch_b // 2, w, col_pitch_b)):
        # Vertical Bitlines (double track)
    ...
    # Fine horizontal bridge line connecting Block A to Block B across boundary
...
# [STRIPPED] def add_unique_sem_noise(image_uint8, seed=24001, is_target=False):
...
# [STRIPPED] 
# [STRIPPED] 
def generate_asymblock_pair1(output_dir="generated_asymblock/pair1", gt_x=500, gt_y=500, seed=24001):
    h, w = 1000, 1000
    ...
    gt_info = {
        "center_x": gt_x,
        "center_y": gt_y,
        "target_name": "Grayscale Asymmetric Dual-Block DRAM - Center GT 500,500 (Pair 1)",
        "pair_id": "pair1",
        "scale_factor": 10.0
    }
    ...
    print(f"[generate_asymblock_pair1] Successfully generated Asymmetric Block DRAM pair 1 {output_dir} | GT: ({gt_x}, {gt_y})")
    return os.path.join(output_dir, "search.png"), os.path.join(output_dir, "target.png"), gt_info


if __name__ == "__main__":
    generate_asymblock_pair1()
