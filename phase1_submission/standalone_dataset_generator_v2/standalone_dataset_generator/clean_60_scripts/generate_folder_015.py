#!/usr/bin/env python3
"""
Generate Pair 3 for generated_new (RGB Silicon Wafer - 3 Identical Bright Chips, Target Nearest to Center: GT 500,405)
======================================================================================================================
Synthesizes 1000x1000x3 uint8 BGR color search image and target reference image matching real optical wafer photo:
  ...
  - 3 microchips on the wafer feature the EXACT SAME bright metallic 484-point joined bus net + X-cross landmark:
      1. (500, 405) - Distance to center (500,500) = 95 px  [NEAREST TO CENTER -> GROUND TRUTH!]
      2. (500, 690) - Distance to center (500,500) = 190 px
      3. (310, 500) - Distance to center (500,500) = 190 px
  - Disambiguation automatically selects the candidate nearest to the center (500, 405)
...
def render_semiconductor_wafer_scene(h=1000, w=1000, gt_x=500, gt_y=405, seed=2003):
    ...
    # 3 Identical Bright Candidate Chips on the Wafer: (500, 405) [Nearest], (500, 690), (310, 500)
    bright_candidates = [(500, 405), (500, 690), (310, 500)]
    ...
    for cy in rows:
        for cx in cols:
            if np.sqrt((cx - center_wafer[0])**2 + (cy - center_wafer[1])**2) <= radius_wafer - 40:
                ...
                is_bright_chip = (cx, cy) in bright_candidates

                if is_bright_chip:
                    # 3 IDENTICAL BRIGHT CHIPS: 484 Points JOINED by metallic trace lines + X-Crosshair
...
# [STRIPPED] def add_heavy_multi_sem_noise_rgb(image_uint8, seed=2003, is_target=False):
...
# [STRIPPED] 
# [STRIPPED] 
def generate_new_pair3(output_dir="generated_new/pair3", gt_x=500, gt_y=405, seed=2003):
    h, w = 1000, 1000
    ...
    gt_info = {
        "center_x": gt_x,
        "center_y": gt_y,
        "target_name": "RGB Silicon Wafer Array (3 Identical Bright Chips, Target Nearest to Center: 500,405) 3",
        "pair_id": "pair3",
        "scale_factor": 10.0
    }
    ...
    wafer_dir = os.path.join("generated_wafer", "pair3")
    ...
    print(f"[generate_new_pair3] Successfully generated RGB wafer pair 3 {output_dir} | GT: ({gt_x}, {gt_y})")
