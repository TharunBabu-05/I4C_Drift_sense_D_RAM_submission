#!/usr/bin/env python3
"""
Generate Pair 2 for Grayscale Single-Sided Stitched Word-Line DRAM (Top GT 500,310)
==================================================================================
Synthesizes 1000x1000 Grayscale SEM image matching Single-Sided Stitched Word-Line DRAM:
  ...
  - Embedded Double-Stitched Landmark at Top (500, 310):
...
def render_stitchedwl_dram_scene(h=1000, w=1000, gt_x=500, gt_y=310, seed=17002):
...
# [STRIPPED] def add_sem_noise_grayscale(image_uint8, seed=17002, is_target=False):
...
# [STRIPPED] 
# [STRIPPED] 
def generate_stitchedwl_pair2(output_dir="generated_stitchedwl/pair2", gt_x=500, gt_y=310, seed=17002):
    h, w = 1000, 1000
    ...
    gt_info = {
        "center_x": gt_x,
        "center_y": gt_y,
        "target_name": "Grayscale Single-Sided Stitched WL DRAM - Top GT 500,310 (Pair 2)",
        "pair_id": "pair2",
        "scale_factor": 10.0
    }
    ...
    print(f"[generate_stitchedwl_pair2] Successfully generated Stitched WL DRAM pair 2 {output_dir} | GT: ({gt_x}, {gt_y})")
    return os.path.join(output_dir, "search.png"), os.path.join(output_dir, "target.png"), gt_info


if __name__ == "__main__":
    generate_stitchedwl_pair2()
