#!/usr/bin/env python3
"""
Generate Pair 1 for Grayscale Vertical BL-Twist DRAM (Center GT 500,500)
========================================================================
...
def render_bltwist_dram_scene(h=1000, w=1000, gt_x=500, gt_y=500, seed=20001):
    ...
    for col_idx, cx in enumerate(range(col_pitch // 2, w, col_pitch)):
        # Generate smooth twisted BL polyline
        pts_left = []
        pts_right = []
        phase = (col_idx % 2) * np.pi

        for y in range(0, h - 35, 4):
            # Zigzag / S-curve offset for BL twist
...
def generate_bltwist_pair1(output_dir="generated_bltwist/pair1", gt_x=500, gt_y=500, seed=20001):
    h, w = 1000, 1000
    ...
    gt_info = {
        "center_x": gt_x,
        "center_y": gt_y,
        "target_name": "Grayscale Vertical BL-Twist DRAM - Center GT 500,500 (Pair 1)",
        "pair_id": "pair1",
        "scale_factor": 10.0
    }
    ...
    print(f"[generate_bltwist_pair1] Successfully generated Vertical BL-Twist DRAM pair 1 {output_dir} | GT: ({gt_x}, {gt_y})")
    return os.path.join(output_dir, "search.png"), os.path.join(output_dir, "target.png"), gt_info


if __name__ == "__main__":
    generate_bltwist_pair1()
