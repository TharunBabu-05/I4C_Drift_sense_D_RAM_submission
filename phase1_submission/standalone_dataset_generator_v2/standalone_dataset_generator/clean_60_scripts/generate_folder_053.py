#!/usr/bin/env python3
"""
Generate Pair 1 for Grayscale BEST Buried-Strap Trench DRAM (Center GT 500,500)
================================================================================
Synthesizes 1000x1000 Grayscale SEM image matching BEST Buried-Strap Trench DRAM:
  ...
  - Stored in Script_and_their_images/053/
"""
...
def render_besttrench_dram_scene(h=1000, w=1000, gt_x=500, gt_y=500, seed=22001):
    ...
    for wy in range(row_pitch // 2, h - 40, row_pitch):
        ...
        # Transistor contact via nodes at WL-BL intersections
        for cx in range(unit_w // 2, w + unit_w, unit_w):
    ...
    # Fine horizontal bridge line connecting adjacent unit cells
...
# [STRIPPED] def add_unique_sem_noise(image_uint8, seed=22001, is_target=False):
...
# [STRIPPED] 
# [STRIPPED] 
def generate_besttrench_pair1(output_dir="generated_besttrench/pair1", gt_x=500, gt_y=500, seed=22001):
    h, w = 1000, 1000
    ...
    gt_info = {
        "center_x": gt_x,
        "center_y": gt_y,
        "target_name": "Grayscale BEST Buried-Strap Trench DRAM - Center GT 500,500 (Pair 1)",
        "pair_id": "pair1",
        "scale_factor": 10.0
    }
    ...
    print(f"[generate_besttrench_pair1] Successfully generated BEST Trench DRAM pair 1 {output_dir} | GT: ({gt_x}, {gt_y})")
    return os.path.join(output_dir, "search.png"), os.path.join(output_dir, "target.png"), gt_info


if __name__ == "__main__":
    generate_besttrench_pair1()
